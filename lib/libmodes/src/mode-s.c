...
// i.e. msg[4] MSB), matching readsb's getbit()/getbits() convention so
// bit numbers from readsb's decodeESOperationalStatus() can be
// transcribed directly (patch: sensorpod).
static int me_getbit(unsigned char *msg, int bitnum) {
  int bi = bitnum - 1;
  int by = 4 + (bi >> 3);
  int mask = 1 << (7 - (bi & 7));
  return (msg[by] & mask) != 0;
}

static unsigned me_getbits(unsigned char *msg, int firstbit, int lastbit) {
  unsigned value = 0;
  int i;
  for (i = firstbit; i <= lastbit; i++) {
    value = (value << 1) | (unsigned)me_getbit(msg, i);
  }
  return value;
}
...
void mode_s_decode(mode_s_t *self, struct mode_s_msg *mm, unsigned char *msg) {
  ...
  // Check CRC and fix single bit errors using the CRC when possible (DF 11 and
  // 17).
  ...
  // Decode extended squitter specific stuff. DF18 CF 0/1/2/6 (ADS-B/TIS-B
  // rebroadcast via ground station) carries the same ME payload layout as
  // DF17, so it is aliased into the same decode path. (patch: sensorpod)
  int df18_cf = msg[0] & 7;
  int is_df18_es = (mm->msgtype == 18) && (df18_cf == 0 || df18_cf == 1 ||
                                           df18_cf == 2 || df18_cf == 6);

  if (mm->msgtype == 17 || is_df18_es) {
    ...
    if (mm->metype >= 1 && mm->metype <= 4) {
      ...
      if (mm->mesub == 1 || mm->mesub == 2) {
        ...
        // Geometric - barometric altitude difference, 25 ft units, bit 49
        // is sign (patch: sensorpod).
        mm->geo_minus_baro_valid = 1;
        mm->geo_minus_baro_sign = (msg[10] & 0x80) >> 7;
        mm->geo_minus_baro = (msg[10] & 0x7f) * 25;
    ...
    } else if (mm->metype == 31 && mm->mesub == 0) {
      // Operational Status, airborne (mesub 0). Bit numbers are
      // ME-relative (1-based), cross-checked against readsb's
      // decodeESOperationalStatus() (patch: sensorpod). Only versions 1
      // and 2 carry NIC-A/NACp/GVA/SIL - version 0 has none of these, so
      // opstatus_valid is left 0 for it.
      unsigned version = me_getbits(msg, 41, 43);
      mm->opstatus_version = version;
      if (version == 1 || version == 2) {
        mm->opstatus_valid = 1;
        mm->nic_a_valid = 1;
        mm->nic_a = me_getbit(msg, 44);
        mm->nac_p_valid = 1;
        mm->nac_p = me_getbits(msg, 45, 48);
        mm->sil_valid = 1;
        mm->sil = me_getbits(msg, 51, 52);
        if (version == 1) {
          mm->sil_type = 0; // per-hour (version 1 doesn't distinguish)
          mm->nic_baro_valid = 1;
          mm->nic_baro = me_getbit(msg, 53);
        } else {                             // version == 2
          mm->sil_type = me_getbit(msg, 55); // 1 = per-sample
          mm->gva_valid = 1;
          mm->gva = me_getbits(msg, 49, 50);
          mm->nic_baro_valid = 1;
          mm->nic_baro = me_getbit(msg, 53);
        }
      }
    } else if (mm->metype == 28 && mm->mesub == 1) {
      // Aircraft Status, Emergency/Priority (mesub 1). Bit numbers are
      // ME-relative (1-based), cross-checked against readsb's
      // decodeESAircraftStatus() (patch: sensorpod).
      mm->es_emergency_valid = 1;
      mm->es_emergency = me_getbits(msg, 9, 11);

      // ID13 field, 13 bits, MSB (bit 12) first. Bit-to-Gillham-group
      // mapping transcribed from readsb's decodeID13Field() (patch:
      // sensorpod): bit12=C1 bit11=A1 bit10=C2 bit9=A2 bit8=C4 bit7=A4
      // bit6=X(unused) bit5=B1 bit4=D1 bit3=B2 bit2=D2 bit1=B4 bit0=D4.
      unsigned id13 = me_getbits(msg, 12, 24);
      if (id13) {
        int a = ((id13 >> 7) & 1) << 2 |  // A4
                ((id13 >> 9) & 1) << 1 |  // A2
                ((id13 >> 11) & 1);       // A1
        ...
        int d = (id13 & 1) << 2 |         // D4
                ((id13 >> 2) & 1) << 1 |  // D2
                ((id13 >> 4) & 1);        // D1
        mm->es_squawk_valid = 1;
        mm->es_squawk = a * 1000 + b * 100 + c * 10 + d;
      }
    }
  }
...
void mode_s_detect(mode_s_t *self, uint16_t *mag, uint32_t maglen,
  ...
  for (j = 0; j < maglen - MODE_S_FULL_LEN * 2; j++) {
    ...
    // test is not passed.
    if (!(mag[j] > mag[j + 1] && mag[j + 1] < mag[j + 2] &&
          mag[j + 2] > mag[j + 3] && mag[j + 3] < mag[j] &&
          mag[j + 4] < mag[j] && mag[j + 5] < mag[j] && mag[j + 6] < mag[j] &&
          mag[j + 7] > mag[j + 8] && mag[j + 8] < mag[j + 9] &&
          mag[j + 9] > mag[j + 6])) {
      continue;
    }
    ...
    if (mag[j + 11] >= high || mag[j + 12] >= high || mag[j + 13] >= high ||
        mag[j + 14] >= high) {
      continue;
    }
